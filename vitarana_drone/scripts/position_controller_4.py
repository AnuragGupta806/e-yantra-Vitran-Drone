...
class Edrone():
    ...
    def velocity_callback(self, msg):
        self.velocity.velocity_lat = msg.vector.x
        self.velocity.velocity_long = -msg.vector.y 
        # self.theta = np.arctan(velocity_long/velocity_lat) - self.drone_orientation_euler[2]
    ...
    def pid(self):
        ...
        self.error_value[0] = (self.setpoint_location[0] - \
                self.drone_location[0]) - self.velocity.velocity_lat * 0.000013552 * 2/3
        self.error_value[1] = (self.setpoint_location[1] - \
                self.drone_location[1]) - self.velocity.velocity_long * 0.000014245 * 2/3
        self.error_value[2] = (self.setpoint_location[2] - \
                self.drone_location[2])

        while(self.error_value[0]*1.5/0.000013552 > 2.5):
            self.error_value[0] /= 2
        while (self.error_value[1]*1.5/0.000014245  > 2.5):
            self.error_value[1] /= 2
    
        # print(self.error_value)
        for i in range(3):
            self.sum_error_value[i] += self.error_value[i]
        ...
        # v1 = scaling_vector*error_value[0]  --> (x)
        # output0 = pid(v1 - v0)

        
        # PID eqation for latitude(X direction)
        output0 = self.Kp[0]*self.error_value[0] + self.Ki[0] * \
            self.sum_error_value[0] + self.Kd[0]*self.change_in_error_value[0]

        # PID eqation for longitude(negative Y direction)
        output1 = self.Kp[1]*self.error_value[1] + self.Ki[1] * \
            self.sum_error_value[1] + self.Kd[1]*self.change_in_error_value[1]
        # PID equation for altitude(Z direction)        
        output2 = self.Kp[2]*self.error_value[2] + self.Ki[2] * \
            self.sum_error_value[2] + self.Kd[2]*self.change_in_error_value[2]
...
def find_nearest_building():
    ...
    # e_drone.setpoint_final = e_drone.buildings[1]
